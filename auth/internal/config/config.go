...
// В Config будем хранить конфигурацию для запуска сервера
type Config struct {
...
// Конструктор новой конфигурации с параметрами по умолчанию
func NewConfig() *Config {
	...
	// Попытка получить Таймаут для БД из переменной окружения
	dbTimeout := 5
	...
	refreshTokenExpiration := 168 // 7 дней (168 часов)
...
// getEnv получает значение переменной окружения, ключ в качестве аргумента,
// после возращает значение переменной ИЛИ ошибку, если переменная не установлена
